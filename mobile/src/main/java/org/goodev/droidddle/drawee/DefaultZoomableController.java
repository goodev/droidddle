...
public class DefaultZoomableController
  ...
  /* TransformGestureDetector.Listener methods  */
    ...
    @Override
    public void fillXOrY() {
        if (mImageBounds.height() == 0 || mViewBounds.height() == 0 || mListener == null) {
            return;
        }
        Matrix transform = mActiveTransform;
        float scale = Math.min(mViewBounds.width() / mImageBounds.width(), mViewBounds.height() / mImageBounds.height());
        transform.postScale(scale, scale, mViewBounds.centerX(), mViewBounds.centerY());
